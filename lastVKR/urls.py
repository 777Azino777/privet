...
from forum.views import first_pages,forum_page,login_user,Addcomment1,registration,Article_comment,News,Photo,logout_user,registr_user,Photo_banay,Rooms_banay,Sport_banay,Pravila,Inform
...
urlpatterns = [
    ...
    re_path( r'^inform/$', Inform),
    re_path( r'^forum/forum_article/(?P<article_id>\d+)/?$', Article_comment),
    re_path( r'^forum/forum_article/(?P<article_id>\d+)/(?P<page_number>\d+)/?$', Article_comment),
