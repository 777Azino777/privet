...
def Article_comment(request,article_id=1,page_number=1):
    comment_form = CommentForm
    all_comment = Comments.objects.filter(comments_article_id=article_id)
    current_page = Paginator(all_comment, 6)
    args = {}
    ...
    args['page_number'] = current_page.page(page_number)
    args['forum_page'] = 'active'
    return render(request,'forum_article.html',args)

def Inform(request):
    args = {}
    args['inf_page'] = 'active'
    return render(request,'inform.html',args)
